...
import { Hint, HintType } from "../../types/hint";
 ...
 */
const recreateGenerator = (hintType: HintType): ((question: QuestionBase) => string) => {
  switch (hintType) {
    case HintType.LETTER_COUNT:
      return (question: QuestionBase) => `Das gesuchte Tier hat ${question.answer.length} Buchstaben`;
    
    case HintType.FIRST_LETTER:
      return (question: QuestionBase) => `Das gesuchte Tier beginnt mit "${question.answer[0].toUpperCase()}"`;
...
export const generateHintContent = (hint: Hint, question: QuestionBase): string => {
  ...
  if (isDynamicHint(hint)) {
    ...
    if (typeof (hint as any).generator === 'function') {
      ...
      console.log('🔧 [generateHintContent] Recreating generator for type:', hint.type);
  ...
  console.warn('🔧 [generateHintContent] Unknown hint type, returning fallback');
  return 'Hint nicht verfügbar';
};