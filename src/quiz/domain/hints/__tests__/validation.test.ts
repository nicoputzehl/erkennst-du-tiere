...
import { createContextualHint, createFirstLetterHint, createLetterCountHint } from "../factories";
  ...
  describe('checkForContextualHints', () => {
    it('should trigger contextual hint on matching answer', () => {
      const question: QuizQuestion = {
        ...
        status: QuestionStatus.ACTIVE,
        hints: [
          createContextualHint(1, ['jaguar'], 'Fast richtig!')
        ]
      };
      const hintState: HintState = {
        questionId: 1,
      ...
      const triggered = HintUtils.checkForContextualHints('jaguar', question, hintState);
