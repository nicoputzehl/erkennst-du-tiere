...
describe('HintUtils.checkForContextualHints', () => {
  ...
  it('should trigger contextual hint on matching answer', () => {
    const triggered = HintUtils.checkForContextualHints('jaguar', mockQuestion, mockHintState);
  ...
  it('should trigger multiple contextual hints if multiple match', () => {
    ...
    const triggered = HintUtils.checkForContextualHints('big cat', questionWithMultiple, mockHintState);
    
    expect(triggered).toHaveLength(2);
  });

  it('should not trigger already triggered hints', () => {
    const hint = mockQuestion.hints![0] as any; // First contextual hint
    const triggeredHintState = {
      ...mockHintState,
      contextualHintsTriggered: [hint.id]
    };
    
    const triggered = HintUtils.checkForContextualHints('jaguar', mockQuestion, triggeredHintState);
  ...
  it('should handle partial matches', () => {
    const triggered = HintUtils.checkForContextualHints('ist das ein jaguar?', mockQuestion, mockHintState);
    
    expect(triggered).toHaveLength(1);
  });

  it('should return empty array when no hints match', () => {
    const triggered = HintUtils.checkForContextualHints('zebra', mockQuestion, mockHintState);
