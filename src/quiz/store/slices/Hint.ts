...
const generateVisibleHint = (
	hint: Hint,
	question: QuestionBase,
): VisibleHint | null => {
	if (!isStandardHint(hint)) return null;
	if (isLetterCountHint(hint))
		return { type: HintType.LETTER_COUNT, value: getLetterCount(question) };
	return {
...
export const createHintSlice: StateCreator<QuizStore, [], [], HintSlice> = (
	...
	applyHint: async (
		...
		if (!question || !hintState || !globalUserPoints) {
			return {
				success: false,
				error: "Benötigte Daten für den Hinweis nicht gefunden.",
			};
		}
		...
		const costs =
			isAutoFree || HintUtils.isContextualHint(hint)
				? 0
				: (hint as any).cost || 0;
		...
		set((currentState) => {
			...
			// Update visbleHints
			newHintState.visibleHints = visibleHint
				? [...newHintState.visibleHints, visibleHint]
				: newHintState.visibleHints;
	...
	initializeHintState: (quizId: string, questionId: number) => {
		set((state) => {
			if (!state.quizStates[quizId]) {
				console.warn(
					`Quiz with ID ${quizId} not found during hint state initialization.`,
				);
				return state;
			}
