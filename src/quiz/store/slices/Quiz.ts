...
export const createQuizSlice: StateCreator<QuizStore, [], [], QuizSlice> = (
	set,
	get,
) => ({
	// === STATE ===
	...
	solveAllQuizQuestions(quizId: string) {
		...
		if (
			...
			newState.completedQuestions === updatedQuestions.length
		) {
			checkForUnlocks();

			if (showToast) {
				showToast(
					`🎉 Quiz "${newState.title}" vollständig gelöst!`,
					"success",
					4000,
				);
			}
	...
	answerQuestion: async (
		...
		console.log(
			`[QuizSlice] Processing answer for ${quizId}/${questionId}: "${answer}"`,
		);
		...
		if (completed) {
			showToast(
				`🎉 Quiz "${result.newState.title}" completed!`,
				"success",
				4000,
			);
		}
		...
		return {
			...
			nextQuestionId:
				QuizUtils.getNextActiveQuestionId(result.newState) ?? undefined,
			unlockedQuizzes,
		...
		for (const config of Object.values(quizConfigs)) {
			if (config.initiallyLocked && config.unlockCondition) {
				...
				if (isUnlocked) {
					...
					if (
						quiz &&
						!get().pendingUnlocks.some((pu) => pu.quizId === quiz.id)
					) {
...
function createPointTransaction(
	...
	return HintUtils.createPointTransaction(
		type,
		amount,
		reason,
		quizId,
