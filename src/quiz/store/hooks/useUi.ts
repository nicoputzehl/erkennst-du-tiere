...
export function useUI() {
  ...
  // Convenience methods
  const showSuccess = useMemo(() => (message: string, duration?: number) => 
    showToast(message, 'success', duration), [showToast]);
  const showError = useMemo(() => (message: string, duration?: number) => 
    showToast(message, 'error', duration), [showToast]);
  const showInfo = useMemo(() => (message: string, duration?: number) => 
    showToast(message, 'info', duration), [showToast]);
  const showWarning = useMemo(() => (message: string, duration?: number) => 
    showToast(message, 'warning', duration), [showToast]);
  
  
  const getPendingUnlocksCount = useMemo(() => () => 
    pendingUnlocks.filter(unlock => !unlock.shown).length, [pendingUnlocks]);
  
  const checkPendingUnlocks = useMemo(() => () => {
    const unshown = pendingUnlocks.filter(unlock => !unlock.shown);
    
    if (unshown.length > 0) {
      console.log(`[useUI] Found ${unshown.length} unshown unlock notifications`);
      ...
      unshown.forEach((unlock, index) => {
        setTimeout(() => {
          showSuccess(`🎉 "${unlock.quizTitle}" ist jetzt verfügbar!`, 3000);
        }, 300 + index * 500);
      });
    }
