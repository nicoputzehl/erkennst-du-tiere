...
export function useQuiz() {
  ...
  const quizzes = useMemo(() => {
    ...
    console.log(`[useQuiz Hook] Converting ${quizArray.length} quizzes to array. isQuizDataLoaded: ${isQuizDataLoaded}`);
    return quizArray;
  }, [quizzesRecord, isQuizDataLoaded]);
  
  const getQuizById = useMemo(() => (id: string): Quiz | undefined => {
    return quizzesRecord[id];
  }, [quizzesRecord]);
  
  const getQuizState = useMemo(() => (id: string): QuizState | undefined => {
  ...
  const initializeQuizState = useQuizStore((state) => state.initializeQuizState);
  ...
  const getQuizProgressString = useQuizStore((state) => state.getQuizProgressString);
  const getNextActiveQuestion = useQuizStore((state) => state.getNextActiveQuestion);
  ...
  const isQuizCompleted = useMemo(() => (quizId: string): boolean => {
    const state = quizStatesRecord[quizId];
    return state ? state.completedQuestions === state.questions.length : false;
  }, [quizStatesRecord]);

  // Abgeleitete Funktion: Freischaltbeschreibung abrufen
  const getUnlockDescription = useMemo(() => (quizId: string): string | null => {
  ...
  const getStatistics = useMemo(() => () => {
    ...
    const completedQuizzes = allStates.filter(quiz => 
      quiz.completedQuestions === quiz.questions.length
    ).length;
    const totalQuestions = allStates.reduce((sum, quiz) => sum + quiz.questions.length, 0);
    const completedQuestions = allStates.reduce((sum, quiz) => sum + quiz.completedQuestions, 0);
