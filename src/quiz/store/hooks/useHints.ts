...
import { AutoFreeHint, ContextualHint } from '../../types/hint';
...
export const useHints = (quizId: string, questionId: number) => {
  ...
  const markAutoFreeHintAsUsed = useQuizStore((state) => state.markAutoFreeHintAsUsed);
  ...
  const { purchasableHints, autoTriggerHints } = useMemo(() => {
    const purchasable = availableHints.filter(h => 
      h.hint.cost > 0 && h.hint.type !== 'contextual' && h.hint.type !== 'auto_free'
    );
    const autoTrigger = availableHints.filter(h => 
