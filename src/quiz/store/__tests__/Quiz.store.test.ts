...
jest.mock('../../utils/quizCreation', () => ({
  createQuizState: jest.fn((quiz, config) => ({
    ...
    questions: quiz.questions.map((q: any, index: number) => ({
      ...
      status: index < (config?.initialUnlockedQuestions || 0) ? 'active' : 'inactive'
    })),
...
describe('QuizStore', () => {
  ...
  it('should persist and rehydrate selected parts of the store', async () => {
    ...
    expect(stateBeforeRehydration.quizStates[MOCK_QUIZ_CONFIG.id]).toBeDefined();
    expect(stateBeforeRehydration.quizStates[MOCK_QUIZ_CONFIG.id].completedQuestions).toBe(1);
    ...
    expect(stateAfterRehydration.quizStates[MOCK_QUIZ_CONFIG.id].completedQuestions).toBe(1);
    expect(stateAfterRehydration.quizStates).toEqual(stateBeforeRehydration.quizStates);
    expect(stateAfterRehydration.navigationHistory).toEqual(stateBeforeRehydration.navigationHistory);
    expect(stateAfterRehydration.pendingUnlocks).toEqual(stateBeforeRehydration.pendingUnlocks);
  });
  ...
  it('resetAllQuizStates should reset all relevant states', () => {
    ...
    act(() => {
      ...
      useQuizStore.getState().addPendingUnlock('someUnlockId', 'Some Quiz Title');
    ...
    expect(state.quizStates[MOCK_QUIZ_CONFIG.id].questions[0].status).toBe(QuestionStatus.ACTIVE);
    expect(state.pendingUnlocks).toEqual([]);
  ...
  it('onRehydrateStorage should show toast on failed hydration', async () => {
    ...
    expect(showToastMock).toHaveBeenCalledWith('Fehler beim Laden der Daten!', 'error', 5000);
    jest.restoreAllMocks();
  });
});