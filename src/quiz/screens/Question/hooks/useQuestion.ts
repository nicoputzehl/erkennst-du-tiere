...
export const useQuestion = (quizId: string, question: Question) => {
	...
	const handleSubmit = useCallback(async () => {
		...
		try {
			console.log(
				`[useQuestion] Submitting answer for quiz ${quizId}, question ${question.id}`,
			);
			...
			if (result.isCorrect && result.newState) {
				...
				if (result.unlockedQuizzes && result.unlockedQuizzes.length > 0) {
					console.log(
						`[useQuestion] Showing toasts for ${result.unlockedQuizzes.length} unlocked quiz(zes)`,
					);

					// Zeige Toast für jedes freigeschaltete Quiz
					result.unlockedQuizzes.forEach(
						(unlockedQuiz: QuizConfig, index: number) => {
							console.log(
								`[useQuestion] Scheduling toast for "${unlockedQuiz.title}" with delay ${index * 500}ms`,
							);

							// Delay für mehrere Toasts, damit sie nacheinander erscheinen
							setTimeout(() => {
								console.log(
									`[useQuestion] Showing toast for "${unlockedQuiz.title}"`,
								);
								showSuccess(
