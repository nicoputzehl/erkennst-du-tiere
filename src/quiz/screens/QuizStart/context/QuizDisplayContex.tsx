...
export function QuizDisplayProvider({
  ...
  const getProgress = useCallback((quizId: string): number => {
    const progress = getQuizProgress(quizId);
    console.log(`[QuizDisplayProvider] Progress for quiz ${quizId}: ${progress}%`);
    
    // Debug: Direkt den Quiz-Zustand prüfen
    const state = getQuizState(quizId);
    if (state) {
      const calculatedProgress = (state.completedQuestions / state.questions.length) * 100;
      console.log(`[QuizDisplayProvider] Quiz state for ${quizId}: ${state.completedQuestions}/${state.questions.length} = ${calculatedProgress}%`);
    } else {
  ...
  const navigateToQuiz = useCallback(async (quizId: string) => {
    setIsLoading(true);
    try {
      // Quiz-Zustand initialisieren, falls noch nicht geschehen
      const state = await initializeQuizState(quizId);
      if (state) {
        console.log(`[QuizDisplayProvider] Quiz ${quizId} initialized: ${state.completedQuestions}/${state.questions.length} completed`);
      }
      ...
      console.error(`[QuizDisplayProvider] Error navigating to quiz ${quizId}:`, error);
    } finally {
      setIsLoading(false);
    }
