// src/quiz/components/QuizCard/QuizCard.tsx
...
export const ActiveQuizCard = ({ quiz }: { quiz: Quiz }) => {
  const { getQuizProgress, getQuizProgressString, navigateToQuiz } = useQuizDisplay();
  
  const progressPercentage = getQuizProgress(quiz.id);
  const progressText = getQuizProgressString(quiz.id);
      ...
      <View style={[styles.quizCard, styles.locked]}>
        {unlockInfo?.condition && (
          <View style={styles.unlockInfo}>
            <Text>
              {unlockInfo.condition.description}
            </Text>
