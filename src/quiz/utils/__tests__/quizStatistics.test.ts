import { createTestQuizState, quizStateBuilder } from "../../testing/testUtils";
import { calculateCompletionPercentage, calculateQuizProgress, countCompletedQuestions, countCompletedQuizzes, countTotalQuestions, countTotalQuizzes, createProgressString, isCompleted } from "../quizStatistics";
...
describe('Quiz Statistics and Progress Utilities', () => {
  ...
  it('handles quiz states with different properties correctly', () => {
    const quizStates = {
      ...
      'quiz-B': createTestQuizState({ id: 'quiz-B', completedQuestions: 5, questions: Array(5).fill({}) as any }),
    };
    ...
    it('returns 0 if no quizzes are completed', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(5).withCompletedQuestions(3).build(),
        'quiz-2': quizStateBuilder().withQuestions(10).withCompletedQuestions(8).build(),
      };
      expect(countCompletedQuizzes(quizStates)).toBe(0);
    });
    ...
    it('returns 1 if one quiz is completed', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(5).withCompletedQuestions(5).build(), // Completed
        'quiz-2': quizStateBuilder().withQuestions(10).withCompletedQuestions(8).build(),
      };
    ...
    it('returns correct count for multiple completed quizzes', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(5).withCompletedQuestions(5).build(), // Completed
        'quiz-2': quizStateBuilder().withQuestions(10).withCompletedQuestions(8).build(),
        'quiz-3': quizStateBuilder().withQuestions(7).withCompletedQuestions(7).build(), // Completed
        'quiz-4': quizStateBuilder().withQuestions(3).withCompletedQuestions(2).build(),
      };
    ...
    it('handles quizzes with 0 questions correctly (considered completed if completedQuestions is also 0)', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(0).withCompletedQuestions(0).build(), // Completed (0/0)
        'quiz-2': quizStateBuilder().withQuestions(5).withCompletedQuestions(3).build(),
      };
      expect(countCompletedQuizzes(quizStates)).toBe(1);
    });
  });
  ...
  describe('countTotalQuestions', () => {
    ...
    it('returns the correct total for multiple quizzes', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(5).withCompletedQuestions(3).build(),
        'quiz-2': quizStateBuilder().withQuestions(10).withCompletedQuestions(8).build(),
        'quiz-3': quizStateBuilder().withQuestions(7).withCompletedQuestions(7).build(),
      };
    ...
    it('handles quizzes with 0 questions', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(5).withCompletedQuestions(3).build(),
        'quiz-2': quizStateBuilder().withQuestions(0).withCompletedQuestions(0).build(), // Empty Quiz
      };
      expect(countTotalQuestions(quizStates)).toBe(5);
    });
  });
  ...
  describe('countCompletedQuestions', () => {
    ...
    it('returns the correct total for multiple quizzes', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(5).withCompletedQuestions(3).build(),
        'quiz-2': quizStateBuilder().withQuestions(10).withCompletedQuestions(8).build(),
        'quiz-3': quizStateBuilder().withQuestions(7).withCompletedQuestions(0).build(),
      };
    ...
    it('handles quizzes with 0 completed questions', () => {
      const quizStates = {
        'quiz-1': quizStateBuilder().withQuestions(5).withCompletedQuestions(0).build(),
        'quiz-2': quizStateBuilder().withQuestions(10).withCompletedQuestions(0).build(),
      };
      expect(countCompletedQuestions(quizStates)).toBe(0);
    });
  });
  ...
  describe('calculateQuizProgress', () => {
    it('returns 0 if the quiz has no questions', () => {
      const emptyQuiz = quizStateBuilder().withQuestions(0).withCompletedQuestions(0).build();
      expect(calculateQuizProgress(emptyQuiz)).toBe(0);
    });

    it('returns 0 if no questions are completed', () => {
      const notStartedQuiz = quizStateBuilder().withQuestions(10).withCompletedQuestions(0).build();
      expect(calculateQuizProgress(notStartedQuiz)).toBe(0);
    });

    it('returns 100 if all questions are completed', () => {
      const completedQuiz = quizStateBuilder().withQuestions(7).withCompletedQuestions(7).build();
      expect(calculateQuizProgress(completedQuiz)).toBe(100);
    });

    it('returns the correct rounded percentage for partial completion', () => {
      // 50%
      let quiz = quizStateBuilder().withQuestions(10).withCompletedQuestions(5).build();
      expect(calculateQuizProgress(quiz)).toBe(50);

      // 33.33...% -> 33%
      quiz = quizStateBuilder().withQuestions(3).withCompletedQuestions(1).build();
      expect(calculateQuizProgress(quiz)).toBe(33);

      // 66.66...% -> 67%
      quiz = quizStateBuilder().withQuestions(3).withCompletedQuestions(2).build();
      expect(calculateQuizProgress(quiz)).toBe(67);

      // 16.66...% -> 17%
      quiz = quizStateBuilder().withQuestions(6).withCompletedQuestions(1).build();
  ...
  describe('createProgressString', () => {
    ...
    it('returns the correct progress string for a partial quiz', () => {
      const inProgressQuiz = quizStateBuilder().withQuestions(10).withCompletedQuestions(3).build();
      expect(createProgressString(inProgressQuiz)).toBe('3/10');
    });

    it('returns the correct progress string for a completed quiz', () => {
      const completedQuiz = quizStateBuilder().withQuestions(5).withCompletedQuestions(5).build();
      expect(createProgressString(completedQuiz)).toBe('5/5');
    });

    it('returns the correct progress string for a not-started quiz', () => {
      const notStartedQuiz = quizStateBuilder().withQuestions(8).withCompletedQuestions(0).build();
