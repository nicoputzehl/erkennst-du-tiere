...
describe('Quiz Progression Utilities', () => {
  ...
  describe('calculateNewQuestionsAfterCorrectAnswer', () => {
    it('sets the answered question to SOLVED and activates the next INACTIVE question', () => {
      ...
      const newQuestions = calculateNewQuestionsAfterCorrectAnswer(initialQuestions, 1); // Answer q2 (index 1)
    ...
    it('does not modify questions if the index is out of bounds', () => {
      ...
      const newQuestions = calculateNewQuestionsAfterCorrectAnswer(initialQuestions, 99); // Invalid index
  ...
  describe('calculateAnswerResult', () => {
    ...
    it('returns original state and isCorrect: false if answer is incorrect', () => {
      ...
      const { newState, isCorrect } = calculateAnswerResult(initialState, initialState.questions[0].id, 'wrong answer');
    ...
    it('updates state and returns isCorrect: true if answer is correct', () => {
      ...
      const { newState, isCorrect } = calculateAnswerResult(initialState, initialState.questions[1].id, 'correct answer');

      expect(isCorrect).toBe(true);
      expect(newState.completedQuestions).toBe(initialState.completedQuestions + 1); // 1 + 1 = 2
    ...
    it('throws an error if questionId is not found', () => {
      ...
      expect(() => calculateAnswerResult(initialState, 999, 'answer')).toThrow('Frage mit ID 999 nicht gefunden');
    });
    ...
    it('correctly increments completedQuestions when an active question is answered correctly', () => {
      ...
      const { newState } = calculateAnswerResult(initialState, initialState.questions[0].id, 'answer');
      expect(newState.completedQuestions).toBe(1);

      const { newState: secondState } = calculateAnswerResult(newState, newState.questions[1].id, 'answer');
  ...
  describe('sortQuestionsByIds', () => {
    ...
    it('returns the same array if already sorted', () => {
      ...
      expect(sortQuestionsByIds(sortedQuestions).map(q => q.id)).toEqual([1, 2]);
    });
  });
  ...
  describe('findNextUnsolvedQuestionForward', () => {
    ...
    it('finds the next unsolved question when available', () => {
      expect(findNextUnsolvedQuestionForward(questions, 0)).toEqual(questions[1]); // From Q1 (solved)
      expect(findNextUnsolvedQuestionForward(questions, 1)).toEqual(questions[2]); // From Q2 (active)
      expect(findNextUnsolvedQuestionForward(questions, 3)).toEqual(questions[4]); // From Q4 (solved)
    });
  ...
  describe('findNextUnsolvedQuestionBackward', () => {
    ...
    it('finds the next unsolved question when available backwards', () => {
      expect(findNextUnsolvedQuestionBackward(questions, 4)).toEqual(questions[2]); // From Q5 (solved)
      expect(findNextUnsolvedQuestionBackward(questions, 2)).toEqual(questions[0]); // From Q3 (active)
    });
    ...
    it('returns null if no next unsolved question is found backward', () => {
      ...
      expect(findNextUnsolvedQuestionBackward(allSolvedQuestions, 1)).toBeNull();
      expect(findNextUnsolvedQuestionBackward(allSolvedQuestions, 0)).toBeNull(); // Index 0 has no backward
    });
