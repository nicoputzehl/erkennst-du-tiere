import { colognePhonetic, normalizeString, parseWord, postProcessPhoneticCode, preprocessWord } from "../stringManipulation";
...
describe('String and Phonetic Utilities', () => {
  ...
  describe('normalizeString', () => {
    ...
    it('should remove multiple consecutive hyphens', () => {
      expect(normalizeString('test--string---with----hyphens')).toBe('test-string-with-hyphens');
    });
    ...
    it('should remove special characters and symbols', () => {
      expect(normalizeString('Test!@#$€%^&*()_+={}[];:\'\",.<>?/\\|`~`')).toBe('test');
