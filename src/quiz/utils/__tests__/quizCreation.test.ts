...
import { QuestionStatus, UnlockCondition } from "../../types";
import { calculateInitialQuestionStatus, createQuizConfig, createQuizState, createUnlockCondition } from "../quizCreation";
...
describe('Quiz Creation Utilities', () => {
  ...
  describe('createQuizConfig', () => {
    ...
    it('overrides default values with provided config', () => {
      const quiz = createTestQuiz({ id: 'override-quiz', title: 'Override Test' });
      const unlockCond: UnlockCondition = { requiredQuizId: 'prev-quiz', description: 'Test Unlock' };
      const config = createQuizConfig(quiz, {
    ...
    it('merges quiz content with config properties', () => {
      const questions = [createTestQuestion({ id: 1 }), createTestQuestion({ id: 2 })];
  ...
  describe('createUnlockCondition', () => {
    it('creates an unlock condition with a default description', () => {
      ...
      expect(condition.description).toBe('Schließe das Quiz "required-1" ab, um dieses Quiz freizuschalten.');
    });
  ...
  describe('createQuizState', () => {
    it('creates a basic quiz state with default initial unlocked questions', () => {
      const quiz = createTestQuiz({
        id: 'new-state-quiz',
        title: 'New State',
        questions: [createTestQuestion({ id: 1 }), createTestQuestion({ id: 2 }), createTestQuestion({ id: 3 }), createTestQuestion({ id: 4 })],
      });
    ...
    it('creates a quiz state with a custom number of initial unlocked questions', () => {
      const quiz = createTestQuiz({
        ...
        title: 'Custom State',
        questions: [createTestQuestion({ id: 1 }), createTestQuestion({ id: 2 }), createTestQuestion({ id: 3 })],
      });
    ...
    it('sets all questions to active if initialUnlockedQuestions is greater than total questions', () => {
      const quiz = createTestQuiz({
        id: 'all-active-quiz',
        title: 'All Active',
        questions: [createTestQuestion({ id: 1 }), createTestQuestion({ id: 2 })],
      });
